...
export class DerivPublicFeed {
  ...
  handleMessage(event) {
    ...
    if (message.req_id && this.pending.has(message.req_id)) {
      ...
      if (message.error) {
        pending.reject(
          new Error(`${message.error.code || 'Error'}: ${message.error.message || 'Error Deriv'}`)
        );
      } else {
  ...
  requestSubscribe = async (request, callback) => {
    try {
      if (request?.ticks_history) {
        ...
        const tickResponse = await this.send({ ticks: symbol, subscribe: 1 }, 20000);
      ...
      const response = await this.send({ ...request, subscribe: 1 }, 20000);
      callback(response);
